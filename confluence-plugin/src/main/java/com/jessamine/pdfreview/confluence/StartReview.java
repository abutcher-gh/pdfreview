...
public class StartReview extends ConfluenceActionSupport {
	...
	StartReview(BootstrapManager bootstrapManager, PageManager pageManager,
			...
			ThemeManager themeManager, BandanaManager bandanaManager) {
	...
	String indexReview(Page reviewIndexPage, Page reviewPage, User user,
			int state) {
		String reviewId = contentPropertyManager.getStringProperty(
				reviewIndexPage, "pdfreview.maxId");
		try {
			reviewId = Integer.toString(Integer.parseInt(reviewId) + 1);
		} catch (Exception e) {
			reviewId = "1";
		}
		contentPropertyManager.setStringProperty(reviewIndexPage,
				"pdfreview.maxId", reviewId);

		contentPropertyManager
				.setStringProperty(reviewPage, "pdfreview.indexPageId",
						Long.toString(reviewIndexPage.getId()));
		...
		if (reviewPages != null && !reviewPages.isEmpty())
			contentPropertyManager.setTextProperty(reviewIndexPage,
					"pdfreview." + reviewId + ".pages", reviewPages);
		contentPropertyManager.setStringProperty(reviewIndexPage, "pdfreview."
				+ reviewId + ".owner", user.getName());
		contentPropertyManager.setStringProperty(reviewIndexPage, "pdfreview."
				+ reviewId + ".created",
				ReviewStatus.tagDateFormat.format(date));
	...
	public void setPdfcontent(File f) {
		pdfContent = f;
	}
	...
	public String execute() {
		...
		try {
			...
			if (reviewSpace == null) {
				reviewSpace = spaceManager.createSpace(reviewSpaceKey,
						space.getName() + " Reviews",
						"Content reviewing for the '" + space.getName()
								+ "' space.", user);
				if (reviewSpace == null) {
					error = "Cannot create review space '" + reviewSpaceKey
							+ "'.  Get your administrator to do so.";
					return "error";
				}
			...
			if (reviewIndexPage == null) {
				...
				try {
					...
					log("Creation of review index page failed. "
							+ ex.toString());
				}
			...
			if (defaultHome != null) {
				// default home was created by space creation but not
			...
			String indexPageId = contentPropertyManager.getStringProperty(
					reviewPage, "pdfreview.indexPageId");

			log("indexPageId = " + indexPageId);

			String reviewId = indexReview(reviewIndexPage, reviewPage, user,
			...
			Date date = new Date();
			SimpleDateFormat headingDateFormat = new SimpleDateFormat(
					"d MMMM yyyy (yyyyMMdd-HHmmss)");

			String reviewTag = reviewId + "-"
					+ reviewPage.getTitle().replace(" ", "-");

			log("reviewTag = " + reviewTag);
			String reviewHeading = "Review " + reviewId + ": "
					+ headingDateFormat.format(date);

			log("reviewHeading = " + reviewHeading);

			markReviewedPages(reviewPageTree,
					Long.toString(reviewPage.getId()), reviewId, date);

			reviewPage
					.setBodyAsString("<ac:macro ac:name='anchor'><ac:default-parameter>"
							+ reviewId
							+ "</ac:default-parameter></ac:macro>"
							+ "<h1>"
							+ reviewHeading
							+ "</h1>"
							...
							+ (pdfContent == null ? ("<h3>This review includes the following pages</h3>"
									+ "<ac:macro ac:name='unmigrated-inline-wiki-markup'><ac:plain-text-body><![CDATA["
									+ wikiFormatTree(reviewPageTree, "") + "]]></ac:plain-text-body></ac:macro>")
									: "<h3>Review of non-wiki document.</h3>")
							+ "<h3>Review progress</h3>"
							+ "<ac:macro ac:name='unmigrated-inline-wiki-markup'><ac:plain-text-body><![CDATA[{review-progress:id="
							+ reviewId
							+ "}]]></ac:plain-text-body></ac:macro>"
							+ "</ac:rich-text-body></ac:macro>"
							+ "<ac:macro ac:name='column'><ac:parameter ac:name='width'>45%</ac:parameter><ac:rich-text-body>"
							+ "<ac:macro ac:name='viewpdf'><ac:parameter ac:name='width'>100%</ac:parameter><ac:default-parameter>"
							+ reviewTag
							+ ".pdf</ac:default-parameter></ac:macro>"
							+ "</ac:rich-text-body></ac:macro>"
							+ "</ac:rich-text-body></ac:macro>"
							+ "<hr />"
							+ existingContent);
			...
			File file;
			if (pdfFile != null)
				file = new File(pdfFile.replace(
						bootstrapManager.getWebAppContextPath() + "/download",
						bootstrapManager.getConfluenceHome()));
			...
			try {
				...
				log("Failed to update review page '" + reviewPage.getTitle()
						+ "'.  " + ex.toString());
				return "error";
			}
			// catch (HibernateSystemException ex)
