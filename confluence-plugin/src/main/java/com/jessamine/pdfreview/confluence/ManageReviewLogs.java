...
public class ManageReviewLogs extends ConfluenceActionSupport {
	...
	public String execute() {
		...
		return "error";
		/*
		 * // FIXME: ugly catch all try {
		 * 
		 * User user = AuthenticatedUserThreadLocal.getUser(); if (user == null)
		 * { error = "Not authenticated.  Cannot proceed."; return "error"; }
		 * Space space = spaceManager.getSpace(spaceKey); if (space == null) {
		 * error = "No such space '" + spaceKey + "'"; return "error"; } Space
		 * reviewSpace = spaceManager.getSpace(reviewSpaceKey); if (reviewSpace
		 * == null) { reviewSpace = spaceManager.createSpace( reviewSpaceKey,
		 * space.getName() + " Reviews", "Content reviewing for the '" +
		 * space.getName() + "' space.", user); if (reviewSpace == null) { error
		 * = "Cannot create review space '" + reviewSpaceKey +
		 * "'.  Get your administrator to do so."; return "error"; }
		 * 
		 * List<SpacePermission> perms = new ArrayList<SpacePermission>();
		 * 
		 * perms.add(SpacePermission.createAnonymousSpacePermission(SpacePermission
		 * .VIEWSPACE_PERMISSION, reviewSpace));
		 * perms.add(SpacePermission.createGroupSpacePermission
		 * (SpacePermission.CREATEEDIT_PAGE_PERMISSION, reviewSpace,
		 * "confluence-users"));
		 * perms.add(SpacePermission.createGroupSpacePermission
		 * (SpacePermission.CREATE_ATTACHMENT_PERMISSION, reviewSpace,
		 * "confluence-users"));
		 * perms.add(SpacePermission.createGroupSpacePermission
		 * (SpacePermission.COMMENT_PERMISSION, reviewSpace,
		 * "confluence-users"));
		 * 
		 ...
		 * ctx.setUpdateLastModifier(false); ctx.setMinorEdit(true);
		 * 
		 * String pageLinks; // create wiki tree to link back to real pages
		 * 
		 * for (String s : tree) { Page p = pageManager.getPage(reviewSpaceKey,
		 * s);
		 * 
		 * log("Page "+p+" ["+s+"]");
		 * 
		 * if (p == null) { p = new Page(); p.setSpace(reviewSpace);
		 * p.setTitle(s); p.setContent(""); try {
		 * pageManager.saveContentEntity(p, ctx); } catch (Exception ex) {
		 * log("Tree creation "
		 * +reviewPath+" failed at page '"+s+"'. "+ex.toString()); } // catch
		 * (HibernateSystemException ex) { // PropertyValueException pve =
		 * (PropertyValueException) ex.getCause(); //
		 * System.err.println("Class: " + pve.getPersistentClass().getName());
		 * // System.err.println("Property: " + pve.getPropertyName()); // } if
		 * (reviewPage != null) { reviewPage.addChild(p);
		 * pageManager.saveContentEntity(reviewPage, ctx); } } reviewPage = p; }
		 * 
		 * if (reviewPage == null) { error = "Could not form review page.";
		 * return "error"; }
		 * 
		 * ctx.setMinorEdit(false); ctx.setUpdateLastModifier(true);
		 * 
		 ...
		 * 
		 * String latestReview =
		 * contentPropertyManager.getStringProperty(reviewPage,
		 * "pdfreview.latest");
		 * 
		 * log("latestReview = " +latestReview);
		 * 
		 * Page oldPage = null; if (latestReview != null) oldPage = (Page)
		 * reviewPage.clone();
		 * 
		 * try { latestReview = Integer.toString(Integer.parseInt(latestReview)
		 * + 1); } catch (Exception e) { latestReview = "1"; }
		 * 
		 * log("latestReview now = " +latestReview);
		 * 
		 * contentPropertyManager.setStringProperty(reviewPage,
		 * "pdfreview.latest", latestReview);
		 * contentPropertyManager.setStringProperty(reviewPage, "pdfreview." +
		 * latestReview, reviewPages);
		 * 
		 * log("SetProp " +latestReview);
		 * 
		 * Date date = new Date(); SimpleDateFormat headingDateFormat = new
		 * SimpleDateFormat("d MMMM yyyy (yyyyMMdd-hhmmss)");
		 * 
		 * String reviewTag = reviewPage.getTitle().replace(" ","-") + "-" +
		 * latestReview + "-" + tagDateFormat.format(date); log("reviewTag = "
		 * +reviewTag); String reviewHeading = "Review " + latestReview + " - "
		 * + headingDateFormat.format(date);
		 * 
		 * log("reviewHeading = " +reviewHeading);
		 * 
		 * markReviewedPages(reviewPageTree, Long.toString(reviewPage.getId()),
		 * latestReview, date);
		 * 
		 * reviewPage.setContent( "h1. " + reviewHeading +
		 * "{anchor:"+latestReview+"}\n" + "{section:border=true}\n" +
		 * "{column:width=50%}\n" +
		 ...
		 * log("content updated");
		 * 
		 * File file = new File(pdfFile.replace("/download",
		 * bootstrapManager.getConfluenceHome()));
		 * 
		 * Attachment attachment = new Attachment(reviewTag+".pdf",
		 * "application/pdf", file.length(), "Snapshot for review " +
		 * latestReview + " of " + reviewPage.getTitle());
		 * 
		 ...
		 * log("updated data");
		 * 
		 * in.close(); in = null; file = null;
		 * 
		 * try { if (oldPage == null) pageManager.saveContentEntity(reviewPage,
		 * ctx); else pageManager.saveContentEntity(reviewPage, oldPage, ctx); }
		 * catch (Exception ex) {
		 * log("Failed to update review page '"+reviewPage
		 * .getTitle()+"'.  "+ex.toString()); return "error"; } // catch
		 * (HibernateSystemException ex) // { // PropertyValueException pve =
		 * (PropertyValueException) ex.getCause(); //
		 ...
		 * done = true;
		 * 
		 * if (error == null || error.isEmpty()) return "success"; else return
		 * "error";
		 * 
		 * } catch (Exception e) { StringWriter stack = new StringWriter();
		 * e.printStackTrace(new PrintWriter(stack)); error = e.toString() +
		 * "\n" + stack.toString(); return "error"; }
		 */
	}
}
